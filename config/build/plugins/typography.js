...
const typographyElements = {
	...
	'p-sm':         [ ".faux-p-sm" ],
	'p-xs':         [ ".faux-p-xs" ],
	'list':         [ "ul:not(.list-none)", "ol:not(.list-none)" ],
	'list-1':         ":is(ul, ol):not(.list-none) > li",
...
const generateRuleSetsByScreen = (screenSize, selector, rules) => {
	ruleSetsByScreen[screenSize] = {
		...ruleSetsByScreen[screenSize],
		[selector]: {
			...(ruleSetsByScreen[screenSize] && { ...ruleSetsByScreen[screenSize][selector] }),
			...rules
		},
	}
}
...
exports.typographyElementsList = Object.values(typographyElements).flat()

exports.plugin = plugin(function({ addBase, theme }) {
	...
	if (typography.styles) {
		Object.entries(typography.styles).forEach(([element, settings]) => {
			...
			generateRuleSetsByScreen('base', selector, {
				...(settings.fontFamily && { fontFamily: theme('fontFamily.' + settings.fontFamily) }),
				...(settings.fontWeight && { fontWeight: theme('fontWeight.' + settings.fontWeight) }),
				...(settings.letterSpacing && { letterSpacing: theme('letterSpacing.' + settings.letterSpacing) }),
				...(settings.uppercase && { textTransform: 'uppercase' }),
				...(settings.fontSize && settings.lineHeight && { paddingTop: '1px' }),
				...(settings.fontSize && settings.lineHeight && { paddingBottom: '1px' }),
			})
			...
			if (settings.lineHeight) {
				Object.entries(settings.lineHeight).forEach(([screen, lineHeight]) => {
					...
					generateRuleSetsByScreen(screenSize, selector + '::before', {
						...
						height: 0,
						display: 'block',
						marginTop: 'calc(-' + calculateOffset(lineHeight, getOffset(settings.fontFamily)[topOffset]) + 'em - 1px)',
					})
					generateRuleSetsByScreen(screenSize, selector + '::after', {
						...
						height: 0,
						display: 'block',
						marginBottom: 'calc(-' + calculateOffset(lineHeight, getOffset(settings.fontFamily).baseline) + 'em - 1px)',
					})
					...
					if (element.includes('list-')) {
						...
						generateRuleSetsByScreen(screenSize, selector + ' > ul::before, ' + selector + ' > ol::before', {
							content: '""',
							width: 0,
							height: 0,
							display: 'block',
						...
						generateRuleSetsByScreen(screenSize, selector + ' > .bullet::before, ' + selector + ' > .bullet::after', {
							marginTop: 'calc(-' + calculateOffset(lineHeight, getOffset(settings.fontFamily)[topOffset]) + 'em)',
							marginBottom: 'calc(-' + calculateOffset(lineHeight, getOffset(settings.fontFamily).baseline) + 'em)',
						})
					}
	...
	const mediaRuleSets = sortedMediaScreens.reduce((acc, screen) => ({
		...acc,
